...
enum token_types {
    ...
    JSE_TOKEN_OPEN_OBJ,
    JSE_TOKEN_CLOSE_OBJ,
    JSE_TOKEN_OPEN_ARRAY,
    JSE_TOKEN_CLOSE_ARRAY,
    JSE_TOKEN_COLON,
    JSE_TOKEN_COMMA,
    JSE_TOKEN_STRING,
    JSE_TOKEN_NUMBER,
...
char *jse_get(JSENSE *j, char *label) {
    ...
    if (tec_string_length(label) >= JSE_MAX_LABEL_SIZE) {
        tec_error("label passed on to jse_get cannot be bigger than %d, incl. NULL character",
                  JSE_MAX_LABEL_SIZE);
        j->last_returned = NULL;
        return NULL;
    }
    ...
    if (tmp) {
        ...
        j->last_returned = tmp;
        if (index != -1) j->last_returned_index = index;
        return tmp->value;
    }
...
char *jse_get_next(JSENSE *j) {
    ...
    if (j->first_token_type) return NULL;

    if (!j->last_returned) return NULL;
...
void jse_free(JSENSE *j) {
    ...
    if (j->root) j_free(j->root);
    if (j->from_file) free(j->buffer);
    free(j);
        ...
        switch (token_type) {
            ...
            case JSE_TOKEN_OPEN_OBJ:
                ...
                switch (prev_token_type) {
                    ...
                    case JSE_TOKEN_OPEN_OBJ:
                        tec_string_copy(j->error, "Two open object characters in a row. Not valid json.",
                                        JSE_ERROR_SIZE);
                        return;
                        break;
                    case JSE_TOKEN_CLOSE_OBJ:
                        tec_string_copy(
                            j->error,
                            "Open object character follows closed one without comma. Not valid json.",
                            JSE_ERROR_SIZE);
                        return;
                        break;
                    case JSE_TOKEN_CLOSE_ARRAY:
                        tec_string_copy(j->error,
                                        "Open object character follows closed array character without comma. "
                                        "Not valid json.",
                                        JSE_ERROR_SIZE);
                        return;
                        break;
                    ...
                    case JSE_TOKEN_LITERAL:
                        tec_string_copy(j->error,
                                        "Open object character immediately follows string/number/literal. "
                                        "Not valid json.",
                                        JSE_ERROR_SIZE);
                        return;
                        break;
            ...
            case JSE_TOKEN_CLOSE_OBJ:
                ...
                stack_positions[stack_index] = 0;
                stack_index -= 1;
                if (stack_index >= 0) st = stack_pointers[stack_index];
                prev = st;
            ...
            case JSE_TOKEN_OPEN_ARRAY:
                ...
                switch (prev_token_type) {
                    ...
                    case JSE_TOKEN_OPEN_OBJ:
                        tec_string_copy(j->error,
                                        "Open array character follows open object character. Not valid json.",
                                        JSE_ERROR_SIZE);
                        return;
                        break;
                    case JSE_TOKEN_CLOSE_OBJ:
                        tec_string_copy(j->error,
                                        "Open array character follows closed object character without comma. "
                                        "Not valid json.",
                                        JSE_ERROR_SIZE);
                        return;
                        break;
                    case JSE_TOKEN_CLOSE_ARRAY:
                        tec_string_copy(j->error,
                                        "Open array character follows closed array character without comma. "
                                        "Not valid json.",
                                        JSE_ERROR_SIZE);
                        return;
                        break;
                    ...
                    case JSE_TOKEN_LITERAL:
                        tec_string_copy(
                            j->error,
                            "Open array character immediately follows string/number/literal. Not valid json.",
                            JSE_ERROR_SIZE);
                        return;
                        break;
                    default:
