...
int tec_string_equal(char *str1, char *str2) {
    ...
    if (!str1 && str2 && !*str2) return 1;
    if (!str2 && str1 && !*str1) return 1;
    if (!str1) return 0;
    if (!str2) return 0;
...
int tec_char_is_white_space(char c) {
    if (c == 32 || c == 9 || c == 10 || c == 11 || c == 12 || c == 13) return 1;
    return 0;
...
int tec_dir_make(char *path) {
    ...
    */
    ...
    if (tec_string_length(path_cpy) > MAX_PATH - 13) return 0;
    if (!tec_string_is_plain_ascii(path_cpy) || tec_string_find_char(path_cpy, ' ') != -1) return 0;
...
void *t_sys_launch_process_error(void *data) {
    ...
    if (!tp_output_error_cb) return 0;
