...
/*tec_string_equal:
        ...
        returns 0 otherwise
        will also return 1 if they are both empty strings (either because they are NULL pointers or there is
   no data in the string) this function is more intuitive than strcmp() as this latter function returns 0 when
   strings are equal
*/
...
tec_string_copy_with_ellipsis:
        ...
        these functions guarantee that at most size - 1 bytes will be written to destination plus a NULL
character this prevents buffer overflows this guarantees you get a NULL terminated string in destination
(unlike strncpy) this function will not cut off the copying in the middle of a UTF8 codepoint when out of
space ellipsis is those "..." added at end of string when space is insufficient for best performance, use
tec_string_copy returns 1 if all was copied right returns 2 if source could not be fully copied returns 0 in
case of error naturally, to use ellipsis, destination needs to be 4 bytes or longer
...
*/
tec_process *tec_sys_launch_process(char *cmd, void (*func_out)(), void *data_out, void (*func_error)(),
                                    void *data_error);
